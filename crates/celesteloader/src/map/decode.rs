...
fn read_bool(buffer: &[u8]) -> Result<(bool, &[u8])> {
...
fn read_bytes(buffer: &[u8], n: usize) -> Result<(&[u8], &[u8])> {
    if n > buffer.len() {
        return Err(Error::EOF);
    }
...
fn read_run_length_encoded(buffer: &[u8]) -> Result<(String, &[u8])> {
...
fn get_var_length(mut buffer: &[u8]) -> Result<(usize, &[u8])> {
    let mut res: usize = 0;
    let mut count = 0;
    loop {
        let byte;
...
fn look<'a>(buffer: &'a [u8], lookup: &[&'a str]) -> Result<(&'a str, &'a [u8])> {
...
fn decode_element<'a>(mut buffer: &'a [u8], lookup: &[&'a str]) -> Result<(Element<'a>, &'a [u8])> {
    let name;
...
fn decode_value<'a>(buffer: &'a [u8], ty: u8, lookup: &[&'a str]) -> Result<(Value<'a>, &'a [u8])> {
